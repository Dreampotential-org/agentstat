...
$("#y-type").html(
  ' House <i class="fas fa-caret-down" aria-hidden="true"></i>'
);
...
function fillIn1() {
  ...
  if (results.city == undefined) {
    ...
    load_agent(true);
  } else $("#city-tab").text(results.city);
  load_agent(true);
...
$("#myButton1").click(function () {
  ...
  if (search_zipcode && $("#y-address").text() == "ZipCode ") {
    new_params.push("zipcode=" + search_zipcode);
  } else if ("zipcode" in newData) {
    new_params.push("zipcode=" + newData.zipcode);
  }
  ...
  if ("state" in newData) {
    new_params.push("state=" + newData.state);
  }
...
function init_maps() {
  ...
  if (inputs.length > 1) {
    ...
    var input_map_bottom =
      document.getElementsByClassName("maps_input_maps")[1];
